...
def get_racing_reference_standings(season: int, series_id="W") -> pd.DataFrame:
    ...
    for i in tqdm(table_rows):
        ...
        race_num = int(str(
            i.find('div', {'class': 'race-number', 'role': 'cell'}).text).replace(" ", ""))
        row_df['race_num'] = race_num
        row_df['race_date'] = i.find(
            'div', {'class': f'date {series_id}', 'role': 'cell'}).text
        ...
        row_df['race_track_name'] = i.find(
            'div', {'class': f'track {series_id}', 'role': 'cell'}).text
        race_track_url = i.find(
            'div', {'class': f'track {series_id}'}).find('a').get("href")
        row_df['race_track_id'] = str(race_track_url).replace(
            'https://www.racing-reference.info/tracks/', '').replace(' ', '')

        row_df['race_car_count'] = int(str(i.find(
            'div', {'class': 'cars no-mobile', 'role': 'cell'}).text).replace(' ', ''))
        row_df['race_winner_name'] = i.find(
            'div', {'class': f'winners {series_id}', 'role': 'cell'}).text
        race_winner_url = i.find(
            'div', {'class': f'winners {series_id}'}).find('a').get("href")
        row_df['race_winner_real_id'] = race_winner_url.replace(
        ...
        row_df['race_winner_make'] = i.find(
            'div', {'class': f'manufacturer {series_id} no-mobile landscape', 'role': 'cell'}).text
        row_df['race_lap_len'] = i.find(
            'div', {'class': 'len no-mobile', 'role': 'cell'}).text
        row_df['race_track_surface'] = i.find(
            'div', {'class': 'sfc no-mobile', 'role': 'cell'}).text
        row_df['race_miles_completed'] = i.find(
            'div', {'class': f'miles no-mobile {series_id} no-right-border', 'role': 'cell'}).text
        ...
        row_df['race_pole_time'] = i.find(
            'div', {'class': 'pole no-mobile no-tablet', 'role': 'cell'}).text
        row_df['race_cautions'] = str(i.find('div', {
                                      'class': 'cautions no-mobile no-tablet', 'role': 'cell'}).text).replace(' ', '')
        row_df['race_speed'] = i.find(
            'div', {'class': 'speed no-mobile no-tablet', 'role': 'cell'}).text
        row_df['race_lead_changes'] = i.find(
            'div', {'class': 'lc no-mobile no-tablet', 'role': 'cell'}).text

        row_df['race_url'] = str(
            i.find('div', {'class': 'race-number'}).find("a").get("href")).replace(' ', '')
...
def get_racing_reference_race_results(season: int, series_id="W"):
    ...
    """
    sched_df = get_racing_reference_schedule(
    ...
    for i in tqdm(range(0, len(race_url_arr))):
        ...
        html_table = soup.find_all(
            'table', {'class': 'tb race-results-tbl'})[0]
        # print(f"\n{html_table}")

        race_id = str(race_results_url).replace(
            "https://www.racing-reference.info/race-results/", "").replace(f"/{series_id.upper()}", "")
        race_id_short = race_id_short_arr[i]

        for j in html_table.find_all('tr'):
            row_df = pd.DataFrame({'season': season, 'series_id': series_id,
                                  'race_id': race_id, 'race_id_short': race_id_short}, index=[0])
            row = j.find_all('td')
            ...
            if len(row) < 2:
                ...
                row_df['driver_nationality'] = str(row[3].find("img").get("src")).replace(
                    '//www.racing-reference.info/wp-content/themes/ndms/images/racing-reference/flags/', '').replace('.png', '')
                try:
...
def main():    
    ...
    for i in range(current_year, current_year+1):
        ...
        if i >= 1987:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_arca_west/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="E")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1948:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_modified/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="N")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2007:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_pintys/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="T")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1996:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/indycar/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="O")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1986:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/indy_lights/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="IL")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/act_late_model/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="AC")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1960:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/supercars_championship/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="V8")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2008:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_peak_mexico/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="MX")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2017:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_fedex_challenge/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="M2")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1956 and i <= 1959:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_convertible/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="V")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1972 and i <= 1973:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_grand_national_east/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="G")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1979 and i <= 1985:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_north_tour/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="NN")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1979 and i <= 2007:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/cart/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="R")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1905 and i <= 1995:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/usac_champ_car/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="UO")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1974 and i <= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/iroc/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="I")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2005 and i <= 2008:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/a1_grand_prix/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="A1")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1996 and i <= 1997:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/natcc/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="NA")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 2005 and i <= 2016:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_whelen_southern_modified/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="M")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1997 and i <= 2014:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/cars_tour/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="US")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1991 and i <= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_southeast_series/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="SE")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1998 and i <= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_midwest_series/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="MW")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1986 and i <= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_southwest_series/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="SW")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1985 and i <= 2006:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/nascar_northwest_series/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="NW")
                if len(results_df) > 0:
                    results_df.to_csv(
        ...
        if i >= 1998 and i <= 2004:
            ...
            if len(sched_df) > 0:
                sched_df.to_csv(
                    f"racing_reference/asa_national_tour/schedule/{i}_schedule.csv", index=False)
            try:
                results_df = get_racing_reference_race_results(
                    season=i, series_id="MW")
                if len(results_df) > 0:
